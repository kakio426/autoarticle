"""
Demo Data Module
Provides mock data for the interactive demo tour.
"""
import uuid
import json
...
def get_demo_articles():
    """
    Returns a list of demo articles for the interactive tour.
    These are pre-defined sample articles that showcase the app's features.
    """
    demo_images_base = "uploaded_images"
...
def load_demo_data_to_db(db_service):
    """
    Load demo articles into the database.
    Returns True if successful, False if already loaded.
    """
    # Check if demo data already exists
    existing = db_service.get_all_articles()
    if existing and len(existing) > 0:
        return False
    
    # Insert demo articles
    for article in get_demo_articles():
        db_service.save_article(article)
    
    return True


def clear_demo_data_from_db(db_service):
    """
